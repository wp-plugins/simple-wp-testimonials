...
if( !class_exists( 'ahFeaturedImages' ) ) {
	...
	class ahFeaturedImages {
		...
		public function __construct( $args ) {
			...
			$this->post_meta_key = 'ahi_'.$this->id.'_'.$this->post_type.'_id';
			...
			add_action( 'admin_init', array( &$this, 'ahi_admin_init' ) );
			add_action( 'add_meta_boxes', array( &$this, 'ahi_add_meta_box' ) );
			add_filter( 'attachment_fields_to_edit', array( &$this, 'ahi_add_attachment_field' ), 11, 2 );

			add_action( 'wp_ajax_set-MuFeaImg-'.$this->id.'-'.$this->post_type, array( &$this, 'ahi_ajax_set_image' ) );
			
			add_action( 'delete_attachment', array( &$this, 'ahi_delete_attachment' ) );
		 ...
		 * @return void 
		 */
		public function ahi_admin_init() {
			wp_enqueue_script(
					'ahi-multiple-featured-images',
					plugin_dir_url( __FILE__ ).'/js/ahi-admin.js',
					'jquery'
			);
		}
		 ...
		 * @return void 
		 */
		public function ahi_add_meta_box() {
			add_meta_box(
				$this->metabox_id,
				$this->labels['name'],
				array( $this, 'ahi_meta_box_content' ),
				$this->post_type,
				'side',
				'low'
			);
		 ...
		 * @return void
		 */
		public function ahi_meta_box_content() {
			global $post;
		   ...
		   echo $this->ahi_meta_box_output( $image_id );
		}
		 ...
		 * @return string 
		 */
		public function ahi_meta_box_output( $image_id = NULL ) {
			global $post_ID;
			
			$output = '';
			
			$setImageLink = sprintf(
					'<p class="hide-if-no-js"><a title="%2$s" href="%1$s" id="ahi_%3$s" class="thickbox">%%s</a></p>',
		 ...
		 * @return string 
		 */
		public function ahi_add_attachment_field( $form_fields, $post ) {
			$calling_id = 0;
		 ...
		 * @return void
		 */
		public function ahi_ajax_set_image() {
			global $post_ID;
			...
			if( $thumb_id == '-1' ) {
				...
				die( $this->ahi_meta_box_output( NULL ) );
			}
			...
			if( $thumb_id && get_post( $thumb_id) ) {
				...
				if( !empty( $thumb_html ) ) {
					...
					die( $this->ahi_meta_box_output( $thumb_id ) );
				}
			}
		 ...
		 * @return void
		 */
		public function ahi_delete_attachment( $post_id ) {
			global $wpdb;
		...
		public static function get_featured_image_id( $image_id, $post_type, $post_id = NULL) {
			...
			return get_post_meta( $post_id, "ahi_{$image_id}_{$post_type}_id", true);
		}
...
function ahi_mfi_get_featured_image_id( $image_id, $post_type, $post_id = NULL ) {
	return ahFeaturedImages::get_featured_image_id( $image_id, $post_type, $post_id );
}

function ahi_mfi_get_featured_image_url( $image_id, $post_type, $size = 'full', $post_id = NULL ) {
	return ahFeaturedImages::get_featured_image_url( $image_id, $post_type, $size, $post_id );
}

function ahi_mfi_get_the_featured_image( $image_id, $post_type, $size = 'full', $post_id = NULL ) {
	return ahFeaturedImages::get_the_featured_image( $image_id, $post_type, $size, $post_id );
}

function ahi_mfi_the_featured_image( $image_id, $post_type, $size = 'full', $post_id = NULL ) {
	return ahFeaturedImages::the_featured_image( $image_id, $post_type, $size, $post_id );
}